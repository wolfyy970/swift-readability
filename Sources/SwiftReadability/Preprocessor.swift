...
final class Preprocessor: ProcessorBase {
    ...
    private let imageCarouselNormalizer: ImageCarouselNormalizer

    init(regEx: RegExUtil = RegExUtil(), extensions: ReadabilityExtensions = []) {
        self.regEx = regEx
        self.extensions = extensions
        self.imageCarouselNormalizer = ImageCarouselNormalizer(regEx: regEx)
    }

    /// Prepare the HTML document for readability to scrape it.
    func prepareDocument(_ document: Document) {
        unwrapNoscriptImages(document)
        if extensions.contains(.rubyNormalization) {
        ...
        if extensions.contains(.imageCarouselRecovery) {
            imageCarouselNormalizer.normalize(document)
        }
    }
